...
def fetch_local_businesses(lat, lon, radius=25000):
    ...
    # Added [timeout:30] directly to the query so the Overpass server knows to allow longer processing
    overpass_query = f"""
    [out:json][timeout:30];
    (
    ...
    headers = {
        'User-Agent': 'TheatreAnalyticsApp/1.4 (Local Testing)'
    }
    
    for url in endpoints:
        try:
            # Increased Python request timeout to 30s to handle larger radii
