...
def get_ons_nomis_profile(lsoa_code, imd_decile, district):
    try:
        # Placeholder for actual Nomis API integration if secrets are provided
        ...
        # West Yorkshire localized ethnicity modeling based on ONS 2021 Census
        dist_clean = str(district).lower() if district else ""
        if "bradford" in dist_clean:
            asian_base = 32 + (10 - decile) * 2  # Higher diversity in deprived deciles
            black_base = 2
            mixed_base = 3
        ...
        # Calculate Dominant Ethnicity and Certainty Score
        eth_dict = {
            "White": eth_white, 
...
def fetch_local_businesses(lat, lon, radius=25000):
    ...
    out center;
    """
    headers = {'User-Agent': 'TheatreAnalyticsApp/2.0 (Local Testing)'}
...
if uploaded_file is not None:
    ...
    st.sidebar.write("By default, the model uses West Yorkshire ACE baselines blended with census factors. You can also supply a custom dataset below.")
        ...
        with st.spinner("Geocoding, calculating census personas, and assessing cultural propensity..."):
            ...
            for idx, row in df.iterrows():
                ...
                if matched and base_geo:
                    ...
                    base_geo["Dominant_Ethnicity"] = profile["Dominant_Eth"]
                    base_geo["Ethnicity_Certainty"] = f"{profile['Eth_Certainty']:.1f}%"
                    ...
                    mapped_rows.append({
                        ...
                        "Dominant_Ethnicity": "Unknown", "Ethnicity_Certainty": "0%"
                    })
    ...
    if st.session_state.processed_data is not None:
        ...
        with top_kpi2:
            st.metric(label="Total Tickets Sold" if qty_col != "None" else "Total Transactions", value=f"{int(total_volume):,}")
        with top_kpi3:
            avg_yield = (total_revenue / total_volume) if total_volume > 0 else 0
            st.metric(label="Average Ticket Yield" if qty_col != "None" else "Average Transaction", value=f"£{avg_yield:,.2f}")
        with top_kpi4:
        ...
        with tab1:
            ...
            if not df_map.empty:
                ...
                fig_map = px.scatter_mapbox(
                    ...
                    hover_data={
                        "Persona": True,
                        "Dominant_Ethnicity": True,
                        "Ethnicity_Certainty": True,
            ...
            with col_p1:
                ...
                persona_metric = st.radio("Analyze Personas By:", ["Revenue", "Ticket Volume"], horizontal=True)
                
                if persona_metric == "Revenue":
                    p_val = sales_col
                    p_title = "Total Revenue (£)"
                else:
                    p_val = '_Volume_'
                    p_title = "Tickets Sold" if qty_col != "None" else "Transactions"
                    
                persona_counts = filtered_df.groupby('Persona')[p_val].sum().reset_index()
                fig_persona = px.pie(persona_counts, names='Persona', values=p_val, title=f"{p_title} by Target Persona", hole=0.4)
                st.plotly_chart(fig_persona, width="stretch")
                
                with st.expander("ℹ️ How are these personas deduced?"):
                    st.markdown("""
                    Personas are calculated using a localized algorithm that cross-references:
                    * **Socio-Economic Deprivation (IMD):** Ranks postcodes from 1 (Most Deprived) to 10 (Least Deprived).
                    * **ONS Census Profiling (LSOA Level):** Evaluates household structure (families vs. singles), age distribution (retirees vs. young professionals), economic status, and education level.
                    
                    **Example Classifications:**
                    * **Affluent Arts Retirees:** IMD Decile 7-10, >20% aged 65+, >18% retired.
                    * **Young Urban Professionals:** IMD Decile 7-10, >28% aged 25-40, >35% degree-educated.
                    * **Hard-Pressed Families:** IMD Decile 1-3, >35% family households.
                    """)
                
            with col_p2:
                st.write("### 🏛️ Cultural Propensity by Local Authority")
                st.write("Compare the average propensity score against actual sales volume in each district.")
                ...
                la_summary = la_summary.sort_values(by='_Volume_', ascending=False)
                
                fig_la = px.bar(
                    la_summary, x='District', y='Tickets %', 
                    title="Percentage of Total Tickets Sold by Authority",
                    labels={'Tickets %': 'Audience (%)'},
                    text_auto='.1f'
                )
                ...
                st.dataframe(
                    la_summary.rename(columns={
                        'Cultural_Propensity': 'Propensity (0-100)', 
                        sales_col: 'Revenue',
                        '_Volume_': 'Volume Sold'
                    }).style.format({
                        'Propensity (0-100)': '{:.1f}', 
                        'Revenue': '£{:,.2f}', 
                        'Tickets %': '{:.1f}%'
            ...
            if not df_imd.empty:
                ...
                for _, row in df_imd.iterrows():
                    profile = get_ons_nomis_profile(row['LSOA_Code'], row['IMD_Decile'], row.get('District', 'Unknown'))
                ...
                with col_e1:
                    st.write("### 🌍 Estimated Ethnicity Breakdown")
                    st.write("Based on ONS Dataset TS021 (Ethnic Group). Because we cannot know a specific buyer's ethnicity, this reflects the statistical distribution of the LSOAs where your tickets were sold.")
                with col_e2:
                    eth_cols = ['Eth_White', 'Eth_Asian', 'Eth_Black', 'Eth_Mixed', 'Eth_Other']
                    eth_weighted = {col: (df_census[col] * df_census[sales_col]).sum() for col in eth_cols}
                    df_eth = pd.DataFrame(list(eth_weighted.items()), columns=['Ethnicity', 'Weighted Score'])
                    df_eth['Ethnicity'] = df_eth['Ethnicity'].str.replace('Eth_', '')
                    fig_eth = px.pie(df_eth, names='Ethnicity', values='Weighted Score', hole=0.4)
        ...
        with tab2:
            ...
            with col2_w2:
                ...
                if not imd_df.empty:
                    ...
                    imd_total = imd_grouped['Audience'].sum()
                    imd_grouped['Percentage'] = (imd_grouped['Audience'] / imd_total) * 100
