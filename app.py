...
def get_local_amenities(lat, lon, radius=1500):
    """Fetches local restaurants, pubs, and cafes from OpenStreetMap Overpass API."""
    overpass_url = "https://overpass-api.de/api/interpreter"
    
    # Using 'nwr' (node, way, relation) catches businesses mapped as building outlines
    overpass_query = f"""
    [out:json];
    (
    ...
    # Overpass requires a User-Agent header, otherwise it blocks the request
    headers = {
        'User-Agent': 'TheatreAnalyticsApp/1.0 (Local Testing)'
    }
    ...
    try:
        ...
        for element in data.get('elements', []):
            ...
            if name != "Unnamed":
                # 'out center' puts coordinates in a 'center' block for buildings
            ...
            if venue_postcode:
                ...
                if v_lat and v_lon:
                    with st.spinner("Plotting roads and discovering local amenities..."):
                        ...
                        amenity_data = get_local_amenities(v_lat, v_lon, radius=1500)
