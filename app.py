...
def get_outcode_data(outcodes):
    """Fallback API call to fetch district-level coordinates."""
    results = {}
    for outcode in outcodes:
        # Outcodes are typically 2-4 alphanumeric characters
...
def extract_outcode(postcode_str):
    """Attempt to safely extract the district/outcode (first half) from a string."""
    pc = str(postcode_str).strip().upper()
    # If it contains a space, take the first part
    if ' ' in pc:
        return pc.split(' ')[0]
    # Otherwise, use regex to grab the first 2-4 characters that look like an outcode
...
if uploaded_file is not None:
    ...
    if st.button("Analyze Data"):
        with st.spinner("Cleaning data and fetching locations from Postcodes.io (Including Fallbacks)..."):
            ...
            # 2. Identify failures and extract Outcodes (Districts) for Fallback
            outcodes_to_fetch = set()
            for idx, row in df.iterrows():
                pc_val = row[postcode_col]
                clean_pc = str(pc_val).replace(' ', '').upper() if pd.notna(pc_val) else ""
                
                # If full postcode isn't in our geo_data, queue up a fallback
                if clean_pc not in geo_data:
                    fallback_val = ""
                    # Use dedicated fallback column if provided
                    if fallback_col != "None" and pd.notna(row[fallback_col]):
                        fallback_val = extract_outcode(row[fallback_col])
                    # Otherwise attempt to salvage the outcode from the failed postcode column
                    elif clean_pc:
            ...
            # Fetch the fallback data
            outcode_data = get_outcode_data(list(outcodes_to_fetch))
            
            # 3. Map both datasets back to rows
            mapped_rows = []
            for idx, row in df.iterrows():
                pc_val = row[postcode_col]
                clean_pc = str(pc_val).replace(' ', '').upper() if pd.notna(pc_val) else ""
            ...
            # Keep rows that have at least some geographic data (Full or Fallback)
            df = df[df['Match_Type'] != "Unmatched"].copy()
            
            st.success("Analysis complete!")
            
            # Summarize the mapping success
            ...
            with tab1:
                ...
                with col1:
                    ...
                    # Filter out the fallback districts from Ward analytics
