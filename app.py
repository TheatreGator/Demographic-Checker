...
if uploaded_file is not None:
    ...
    if st.session_state.processed_data is not None:
        ...
        with tab1:
            ...
            if not df_map.empty:
                # ADDED 'Ward' into the groupby so it pulls into the map correctly
        ...
        with tab2:
            ...
            with col2_w2:
                st.write("### 📊 Revenue by Deprivation Decile (IMD)")
                imd_df = filtered_df[filtered_df['IMD_Decile'] != 'Unknown'].copy()
                if not imd_df.empty:
                    imd_df['IMD_Decile'] = imd_df['IMD_Decile'].astype(int)
                    imd_grouped = imd_df.groupby('IMD_Decile')[sales_col].sum().reset_index().sort_values('IMD_Decile')
                    fig_imd = px.bar(imd_grouped, x='IMD_Decile', y=sales_col, title="Revenue by Deprivation Decile (1 = Most Deprived, 10 = Least)", text_auto='.2s')
