...
def fetch_local_businesses(lat, lon, radius=1500):
    ...
    headers = {
        'User-Agent': 'TheatreAnalyticsApp/1.1 (Local Testing)'
    }
    
    try:
        # Increased timeout slightly to account for heavy OpenStreetMap server loads
            ...
            if name != "Unnamed":
                ...
                if element_lat is not None and element_lon is not None:
                    amenities.append({
                        ...
                        "lat": float(element_lat), # Enforcing numeric format for PyDeck
                        "lon": float(element_lon)  # Enforcing numeric format for PyDeck
                    })
