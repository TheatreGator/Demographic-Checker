...
def get_ons_nomis_profile(imd_decile):
    """
    Simulates ONS Nomis API Census data based on IMD Deprivation mapping.
    Used to build the UI without hitting API rate limits or requiring Nomis Geometry ID mapping.
    """
...
if uploaded_file is not None:
    ...
    if st.session_state.processed_data is not None:
        ...
        with tab1:
            ...
            st.markdown("---")
            st.write("### 🏢 ONS Census & Demographic Profiling (LSOA Proxy)")
            st.caption("Modeled via IMD weighting. Replace `get_ons_nomis_profile()` with live Nomis API keys to fetch real Census 2021 data.")
            
            if not df_imd.empty:
                census_data = []
                for _, row in df_imd.iterrows():
                    profile = get_ons_nomis_profile(row['IMD_Decile'])
                ...
                with col_c1:
                    ...
                    fig_age = px.pie(df_age, names='Age Group', values='Weighted Score', title="Estimated Audience Age Profile", hole=0.4)
                ...
                with col_c2:
                    ...
                    fig_hh = px.bar(df_hh, x='Household Type', y='Weighted Score', title="Estimated Household Composition")
