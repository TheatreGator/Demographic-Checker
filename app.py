...
def fetch_local_businesses(lat, lon, radius=25000):
    ...
    headers = {
        'User-Agent': 'TheatreAnalyticsApp/1.7 (Local Testing)'
    }
...
if uploaded_file is not None:
    ...
    if st.session_state.processed_data is not None:
        ...
        with tab4:
            ...
            st.write("Cross-reference your highest-value audience driving routes against local amenities and billboards to identify prime marketing and partnership opportunities exactly along their travel path.")
            ...
            if venue_postcode:
                v_lat, v_lon = get_venue_coordinates(venue_postcode)
                if v_lat and v_lon:
                    with st.spinner("Plotting roads and filtering businesses & billboards along the routes..."):
                        ...
                        if amenity_data.empty and not raw_amenity_data.empty:
                            st.warning("Found points in the area, but none were located exactly along the top 50 driving routes.")
                        elif raw_amenity_data.empty:
                            st.warning("⚠️ **OpenStreetMap Issue:** The API successfully connected but returned 0 results. It may be heavily rate-limiting requests right now.")
                        ...
                        if not amenity_data.empty:
                            ...
                            selected_types = st.multiselect(
                                "Select categories to display on the map and table:", 
                                available_types, 
                        ...
                        # Split remaining filtered data for different colors
                        if not display_data.empty:
                            ...
                            # 2. Venues Layer (Green Dots)
                            if not venues_df.empty:
                                amenity_layer = pdk.Layer(
                                    "ScatterplotLayer",
                                    data=venues_df,
                                    get_position="[lon, lat]",
                                    get_color="[50, 200, 50, 200]", # Green
                                    get_radius=50,
                                    radius_min_pixels=4, 
                                    pickable=True
                                )
                            ...
                            # 3. Advertising Layer (Orange Dots)
                            if not ads_df.empty:
                                ad_layer = pdk.Layer(
                                    "ScatterplotLayer",
                                    data=ads_df,
                                    get_position="[lon, lat]",
                                    get_color="[255, 165, 0, 220]", # Orange
                                    get_radius=50,
                                    radius_min_pixels=4, 
                                    pickable=True
                                )
                                layers.append(ad_layer)
                            
                        # 4. Venue Layer (Large Blue Dot)
                        ...
                        st.success(f"**Map Key:** \n* **Blue:** Venue ({venue_postcode})\n* **Red Paths:** Top 50 Driving Routes based on Revenue\n* **Green Dots:** Local Restaurants, Pubs, & Cafes along route.\n* **Orange Dots:** Advertising Boards & Billboards along route.")
                        
                        # Data table of local amenities with clickable links
                        if not display_data.empty:
                            ...
                            st.dataframe(
                                ...
                                column_config={
                                    "link": st.column_config.LinkColumn("Website / Map Location")
                                }
                            )
                else:
